...
function varargout=Chile2010(varargin)
Model=varargin{1};
dxgrid=varargin{2};
longT2=varargin{3};
latT2=varargin{4};
...
for R=1:numel(dip1a)
      for j=Model
    %disp([num2str(R),'/',num2str(numel(W)),' Geometry calculation for Chile2010'])
    ...
    % Change dataset depending on the Rake
        if (rake1a(R) <=135 && rake1a(R) >=45)% Dip-slip |reverse fault
            EDGmat=EDGDSREV(j,30);disp ('la1');dx_best=dx_bestDSREV(j,30);
        elseif (rake1a(R) <=-45 && rake1a(R) >=-135)% Dip-slip |normal fault
            EDGmat=EDGDSNORM(j,30);disp ('la2');dx_best=dx_bestDSNORM(j,30);
        elseif (rake1a(R) >-45 && rake1a(R)<=45)%Strike slip
            EDGmat=EDGSS(j,30);disp ('la3');dx_best=dx_bestSS(j,30);
        elseif (rake1a(R)>135 && rake1a(R)<=180)%Strike slip
            EDGmat=EDGSS(j,30);disp ('la4');dx_best=dx_bestSS(j,30);
        elseif (rake1a(R) <-135 && rake1a(R)>=-180)%Strike slip
            EDGmat=EDGSS(j,30);disp ('la5');dx_best=dx_bestSS(j,30);
        end
...
for R=1:numel(dip1a)
    disp([num2str(R),'/',num2str(numel(W)),' Okubo calculation for Maule2010'])
