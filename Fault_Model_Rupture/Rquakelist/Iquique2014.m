%%%%%%%%%% Iquique Chile 2014 earthquake 
%http://www.tectonics.caltech.edu/slip_history/2014_chile/index.html
function varargout=Iquique2014(varargin)
Model=varargin{1};
dxgrid=varargin{2};
longT2=varargin{3};
latT2=varargin{4};
...
for R=1:numel(dip1a)
      for j=Model
   % disp([num2str(R),'/',num2str(numel(W)),' Geometry calculation for Iquique2014'])
...
for R=1:numel(dip1a)
    disp([num2str(R),'/',num2str(numel(W)),' Okubo calculation for Iquique'])
