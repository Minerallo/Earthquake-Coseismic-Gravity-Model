function varargout=Balochistan2013(varargin)
Model=varargin{1};
dxgrid=varargin{2};
longT2=varargin{3};
latT2=varargin{4};
...
for R=1:numel(dip1a)
    for j=Model
        %disp([num2str(R),'/',num2str(numel(W)),' Geometry calculation for Balochistan2013'])
...
for R=1:numel(dip1a)
    disp([num2str(R),'/',num2str(numel(W)),' Okubo calculation for Balochistan2013'])
...
% % title('Chile Beta=0');
% 
% m_proj('Miller Cylindrical','long',[60 70],'lat',[20 30]);
% % fig points
% set(figure,'Position',[1 1 2000 1500]) % [coin_x coin_y hauteur(px) largeur(px)]
% set(gcf,'PaperPositionMode','auto')
% m_pcolor(longT,latT,matot),shading flat;
% colormap(jet);
% colormap(b2r((min(min(matot))),max(max(matot))));
% m_coast('color','k','LineWidth',1);
% m_grid('box','fancy','tickdir','out');colorbar;
% title('Chile Beta=0');
