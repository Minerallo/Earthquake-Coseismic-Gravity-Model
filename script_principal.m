...
%============Goal=================================
% -This code is calculating gravity change or elevation change using the GCMT catalog 
%  and the analytic solution of Okubo 1992

%============Requirements=========================
%To install the earthquakes catalog or GCMT catalog, you need first to download the passive seismology toolbox "Seizmo".
%Either by dowloading the zipfile and then decompressed it
%https://github.com/g2e/seizmo/archive/master.zip
%or by using the Git command 
%git clone git://github.com/g2e/seizmo.git

%Once you have downloaded "Seizmo", in the command Window of Matlab,
%tape "globalcmt_create" to create the GCMT catalog
%then, tape "globalcmt_update" to update the last earthquakes.
...
%% Sources Model of Large Earthquakes
%http://www.tectonics.caltech.edu/slip_history/index.html
...
coef_elag=5;%coef to wider the mesh grid for each earthquake
coef_tile=90;%tile coefficient spliting mesh grid 
...
%You can choose either nodal plan 1 or 2 or the one that give a minimum or maximum result
...
for Mnum=1:size(M,1)
    ...
    % construction of the total grid where all the local grid of each earthquake will be interpolated
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for R=1:numel(Rt)
        Mwvec(R)=Mw(R) ;
        %% Rupture model known
%looking for an well known rupture plan from large earthquake in the space and time selected
        if isempty(defdx)==0
            dxgrid=dxgrid1;
